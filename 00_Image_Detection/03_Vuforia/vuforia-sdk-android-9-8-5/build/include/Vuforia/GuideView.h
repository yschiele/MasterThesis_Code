...
public:
    ...
    */
     ...
     * getImage() will therefore return the new image.
     *
     ...
     * object at the pose returned by getPose().
     *
     ...
     *
     * \note The image is rendered with the latest available camera profile or a
     * default profile if no camera is available. Thus, the guide view image will look
     * different depending on whether the dataset was loaded before or after the
     * device camera was initialized.
     *
     * \note If the guide view's pose is changed, the guide view's image is
     * regenerated and the previous image is invalidated.
     *
