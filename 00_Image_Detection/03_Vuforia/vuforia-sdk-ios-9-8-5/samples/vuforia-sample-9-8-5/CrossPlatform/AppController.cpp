...
namespace
{
    constexpr char licenseKey[] = "";
...
bool AppController::initTrackers()
{
    ...
    {
        LOG("Error: Failed to initialize SmartTerrain.");
        mShowErrorCallback("Error initializing the SmartTerrain");
        return false;
    }
...
bool AppController::loadTrackerData()
{
    ...
    if (mTarget == IMAGE_TARGET_ID) //Hier wird Image Source festgelegt
    {
        mCurrentDataSet = loadAndActivateDataSet("vuforia-native-masterthesis.xml");
        if (mCurrentDataSet == nullptr)
        {
...
bool AppController::startTrackers()
{
    ...
    if(deviceTracker != 0)
    {
        LOG("STARTED SMARTTERRAIN!!");
        deviceTracker->start();
    } else {
        LOG("FAILED TO START deviceTracker!!");
    }
    ...
    if(smartTerrain != 0)
    {
        ...
        LOG("FAILED TO START SMARTTERRAIN!!");
    }
...
void AppController::stopTrackers()
{
    ...
    {
        smartTerrain->stop();
        LOG("Successfully stopped the smartTerrain");
    }
    else
    {
        LOG("Error: Failed to get the smartTerrain from the tracker manager");
    }
}
