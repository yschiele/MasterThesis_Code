...
enum PIXEL_FORMAT {
    ...
    YUV420P = 256,              ///< A color pixel stored in 12 or more bits
                                ///< using discreet Y, U and V planes
    YUYV = 512,                 ///< A color pixel stored in 16 or more bits
...
enum HINT {
    ...
    */
    ...
    /**
     * Configures the device tracker for statically mounted devices, e.g. a mobile device on a tripod looking at an object.
     * In such a case, standard Vuforia fusion tracking will not succeed to initialize for lack of movement.
     * This hint configures the device tracker to return static poses to stabilize tracking e.g. in case of the object being occluded.
     * Valid values for the hint are 0 or 1
     * When set to 0, the device tracker is initialised normally.
     * When set to 1, the device tracker is changed into the static mode.
     * Default is: 0
     * \note This hint can only be set before the DeviceTracker has been initialized
     */
